...
ara.on("CB:action,,call", async json => {
    ...
    await sleep(4000);
    await frhan.blockUser(callerId, "add"); // Block user
    //}
  });
	...
	ara.on('chat-update', async (mek) => {
      ...
      for (let kemem of bad) {
        ...
        if (isOwner) return reply ('untung kamu owner aku :v Btw jan marah yaa')
				ara.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply(`Woyy ${sender.split("@")[0]} Jangan Ngomong Kasar Ngemtod😡`)
				setTimeout(() => {
				 ara.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
				}, 3000)
				setTimeout(() => {
					ara.updatePresence(from, Presence.composing)
					reply("Babay")
		...
		case 'daftar':
			 ...
			 fs.writeFileSync("./database/pengguna/user.json", JSON.stringify(sender));
                	try {
...
play = body.slice(7)
        ...
        case 'ytmp4':
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4?url=${body.slice(7)}&apikey=apivinz`)
